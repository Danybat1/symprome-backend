...
@CrossOrigin("*")
@RestController
@RequestMapping("/api/v1/typePaiement")
public class TypePaiementController {

    private TypePaiementDao typePaiement;
    @GetMapping
    public HttpBodyResponse getAll(){
        HttpBodyResponse bodyResponse=new HttpBodyResponse();
        try{
            bodyResponse.setResponse(typePaiement.findAll());
        }catch (Exception e){
    ...
    public HttpBodyResponse getOne(@PathVariable Integer id){
        HttpBodyResponse bodyResponse=new HttpBodyResponse();
        try{
            bodyResponse.setResponse(typePaiement.findById(id).get());
        }catch (Exception e){
            bodyResponse.setError(e.getMessage());
            bodyResponse.setStatus("Failed");
    ...
    @PostMapping
    public HttpBodyResponse save(@RequestBody TypePaiement s){
       /*/ Role role=roleDao.findById(Utils.users.getId()).get();
        ...
        HttpBodyResponse bodyResponse=new HttpBodyResponse();
        try{
            bodyResponse.setResponse(typePaiement.save(s));
        }catch (Exception e){
            bodyResponse.setError(e.getMessage());
            bodyResponse.setStatus("Failed");
    ...
    @PutMapping
    public HttpBodyResponse update(@RequestBody TypePaiement s){
      /*  Role role=roleDao.findById(Utils.users.getId()).get();
        if(!role.getDescription().equalsIgnoreCase("admin")){
            throw new ResponseStatusException(HttpStatus.FORBIDDEN);
        }*/
